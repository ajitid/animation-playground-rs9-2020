...
import { mixComplex, mixArray, mixObject } from './mixComplex';
...
function detectMixerFactory<T>(v: T): MixerFactory<any> {
  if (typeof v === 'number') {
    ...
    invariant(false, 'Invalid values passed to mixer.');
    return () => () => {};
  }
}

function createMixers<T>(output: T[], ease?: MixEasing, customMixer?: MixerFactory<T>) {
  const mixers: Array<Mix<T>> = [];
  const mixerFactory: MixerFactory<T> = customMixer || detectMixerFactory(output[0]);
...
function interpolate<T>(
  ...
  const interpolator =
    inputLength === 2 ? fastInterpolate(input, mixers) : slowInterpolate(input, mixers);

  return clamp
    ? (pipe(makeInputClamp(input[0], input[inputLength - 1]), interpolator) as Mix<
        number | string | T
      >)
    : interpolator;
}
